...
class RatioTest < Minitest::Test
  ...
  def test_it_opens_on_the_crossing_the_chapters_before_it_held
    assert_in_delta 1.0 / 1.33, opening.fetch(:mu_21), 0.01
  ...
  def test_the_ratio_is_the_one_thing_this_chapter_adds
    refute_includes Refraction.quantities.keys, :mu_21
    assert_includes RelativeIndex.quantities.keys, :mu_21
    assert_equal [ :snells_law, :relative_index ], RelativeIndex.equations.keys
    assert_includes playing.controls, %(data-input="mu_21")
  end
  ...
  def test_naming_the_first_medium_one_leaves_the_second_holding_the_ratio
    ...
    assert_in_delta 1.0, posed[:mu_1], 1e-9
    assert_in_delta opening.fetch(:mu_21), posed[:mu_2], 1e-9
  end
  ...
  def test_the_ratio_is_the_only_thing_to_move
    assert_equal [ :mu_21 ], opening.keys
  ...
  def test_the_ratio_alone_decides_which_way_and_whether
    assert_equal "—", says(mu_21: 1.5).fetch("critical")
    assert_equal "19.47°", says(mu_21: 1.5).fetch("angle of refraction")
    assert_equal "yes", says(mu_21: 0.4).fetch("no refracted ray")
  end

  def test_and_at_one_the_ray_goes_straight_on
    assert_equal "30.00°", says(mu_21: 1.0).fetch("angle of refraction")
  end
  ...
  def test_the_denser_side_is_the_shaded_one_whichever_side_it_is
    ...
    assert_equal [ [ "100", "0.166" ] ], bands(drawn(mu_21: 1.5))
    assert_empty bands(drawn(mu_21: 1.0))
  end
  ...
  def test_and_by_as_much_as_the_chapters_that_name_their_media
    assert_equal bands(drawn(mu_21: 1.33)).first.last,
                 DRAWING.picture(DRAWING.opening.merge(mu_1: 1.0, mu_2: 1.33), settled: {})
