...
class QuantitiesTest < Minitest::Test
  def test_a_quantity_answers_to_its_alias_and_its_full_name
    declared = quantities { quantity :angle, variable: :a }

    assert_equal :angle, declared.quantities.fetch(:a)
    assert_equal :angle, declared.quantities.fetch(:angle)
  end

  def test_a_declared_domain_is_kept_under_the_full_name
    assert_equal 0.0..1.5, quantities { quantity :angle, variable: :a, within: 0.0..1.5 }
                             .domains.fetch(:angle)
  end

  def test_a_quantity_with_no_domain_declares_none
    refute quantities { quantity :depth }.domains.key?(:depth)
  end

  def test_an_alias_is_optional
    declared = quantities { quantity :depth }

    assert_equal({ depth: :depth }, declared.quantities)
  end

  def test_two_laws_naming_the_same_quantity_mean_the_same_variable
    first = law { quantity :angle, variable: :a; quantity :width, variable: :w
                  equation(:one) { a == w } }
  ...
  def test_declaring_quantities_states_no_equations
    refute_respond_to quantities { quantity :angle }, :equations
  end
  ...
  def test_including_quantities_brings_the_quantities
    ...
    assert_equal :angle, scenario.quantities.fetch(:a)
  ...
  def self.catalogue
    Module.new do
      ...
      quantity :angle, variable: :a, within: 0.0..1.5
    end
  end
