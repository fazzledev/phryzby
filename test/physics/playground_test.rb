...
class PlaygroundTest < Minitest::Test
  ...
  def test_a_scenario_is_still_a_scenario
    assert_in_delta 41.6823, playing.posing(**opening).solve(:r_r).in_degrees, 1e-3
  end

  def test_it_starts_where_it_was_told_to
    assert_in_delta 30.0, opening.fetch(:i).in_degrees, 1e-9
    assert_in_delta 1.0, opening.fetch(:mu_2), 1e-9
  end
  ...
  def test_a_sideways_question_poses_the_situation_afresh
    here = playing.posing(**opening.merge(mu_1: 1.5, mu_2: 1.0))
    here.solve(:r_l)

    assert_in_delta 41.8103, here.asking(:i, r_r: 90.deg).in_degrees, 1e-3
  end
  ...
  def test_what_was_solved_along_the_way_is_not_carried_into_it
    ...
    refute_in_delta here[:i].in_degrees, here.asking(:i, r_r: 90.deg).in_degrees, 1e-6
  end

  def test_the_situation_it_was_asked_of_is_left_alone
    here = playing.posing(**opening.merge(mu_1: 1.5, mu_2: 1.0))
    here.asking(:i, r_r: 90.deg)

    assert_in_delta 30.0, here[:i].in_degrees, 1e-9
  end

  def test_a_reading_that_cannot_be_worked_out_says_so
    assert_includes playing.readouts(opening.merge(mu_1: 1.5, mu_2: 1.0, i: 60.deg)), "—"
  end
  ...
  def test_a_quantity_is_written_the_way_the_law_writes_it
    assert_equal :mu_1, playing.written_for(:refractive_index_of_first_medium)
  ...
  def test_a_marked_input_draws_a_notch_at_each_mark
    ...
    assert_includes controls, 'data-set="1.5" data-for="mu_1"'
  end
  ...
  def test_a_reading_says_what_it_is_sitting_on
    _, _, labels = playing.moved(opening.merge(mu_2: 2.42)).split(APART)

    assert_includes labels, "2.42<small>diamond</small>"
  end

  def test_and_says_nothing_between_them
    _, _, labels = playing.moved(opening.merge(mu_2: 2.0)).split(APART)
    mu_2 = labels.split(2.chr).find { |pair| pair.start_with?("mu_2") }

    refute_includes mu_2, "<small>"
  end
